...
class Banner extends Model
{
	...
	function edit_status($params){
		$result = $this->isUpdate(true)->allowField(true)->update($params);
		if($result){
			return true;
		}else{
			return false;
	...
	*/
	function get_list($where = '',$order = 'id desc',$page_size=15,$whith_page=0){
		...
		if($whith_page == 0){
			$banners = $this->where($where)->order($order)->limit($page_size)->select();
			return $banners;
		}
		
		$banners = $this->where($where)->order($order)->paginate($page_size);
		if($whith_page == 1){
			return $banners;
		}
		
		$banners = $banners->toArray();
		foreach ($banners['data'] as $k => $v) {
			$banners['data'][$k]['category_name'] = cache('categorys')[$v['category_id']]['name'];
		}
		return $banners;
	}
	/**
	...
	*/
	...
	// 查询指定栏目下的Banner数量
	public function get_num_by_cateid ($cate_id, $is_use=1) {
		return $this->where(['category_id' => $cate_id])->where(['is_use' => $is_use])->count();
	}
