        ...
        <table class="list-table">
          ...
          <?php foreach ($category_list as $v) { ?>
            <tr>
              ...
              <?php if($v['is_menu'] != 1){ ?>
                <a href="javascript:void(0)" class="list-switch list-switch-off" category-id="<?php echo $v['id'] ?>"  category-status="1" title="点击开启"><i class="layui-icon">&#x1006;</i></a>
              <?php }else{ ?>
                <a href="javascript:void(0)" class="list-switch list-switch-on" category-id="<?php echo $v['id'] ?>"  category-status="0" title="点击关闭"><i class="layui-icon">&#xe605;</i></a>
              <?php } ?>
...
layui.use(['element', 'layer', 'form'], function(){
  ...
  jq('.list-switch').click(function(){
    var id = jq(this).attr('category-id');
    var status = jq(this).attr('category-status');
      loading = layer.load(2, {
        shade: [0.2,'#000'] //0.2透明度的白色背景
      });
      var row = jq(this);
      jq.post('{:url("category/menu_switch")}',{'id':id, 'status':status},function(data){
        if(data.code == 200){
          if(row.hasClass('list-switch-off')){
            row.removeClass('list-switch-off').find('.layui-icon').html('&#xe605;');
            row.attr('title','点击关闭');
            row.attr('category-status', 0);
          }else{
            row.addClass('list-switch-off').find('.layui-icon').html('&#x1006;');
            row.attr('title','点击开启');
            row.attr('category-status', 1);
          }
          layer.close(loading);
